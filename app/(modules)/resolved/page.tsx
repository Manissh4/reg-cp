...
import { FeedbackModal } from "@/components/provide-feedback";
...
export default function ResolvedGrievancePage() {
  ...
  useEffect(() => {
    setHeader({
      title: "List of Resolved Grievance",
      leftIcon: (
        <svg
          width="24"
          height="24"
  ...
  const handleFeedbackClick = (grievanceId: string) => {
  ...
  const handleFeedbackSubmit = (feedback: any) => {
    console.log(
      "Feedback submitted for grievance",
      selectedGrievanceId,
      feedback
    );
  };

  const handleFeedbackReopen = (feedback: any) => {
    console.log(
      "Reopen requested for grievance",
      selectedGrievanceId,
      feedback
    );
  };
        ...
        <div className="space-y-4">
          {grievances.map((grievance, index) => (
            <GrievanceCard
              key={index}
              {...grievance}
              onButtonOneClick={() => handleFeedbackClick(grievance.id)}
              buttonOne={{
                label: "Provide Feedback",
                borderColor: "#235e90",
                textColor: "#ffffff",
                backgroundColor: "#235e90",
              }}
              buttonTwo={{
