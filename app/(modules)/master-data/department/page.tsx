...
export default function Page() {
    ...
    const [activeTab, setActiveTab] = useState<"Department" | "Category" | "Sector" | "SDG">("Department")
    ...
    return (
        <div className="w-full h-full bg-[#F4F3F9] p-6 flex flex-col gap-2">
            <div className="w-full bg-white rounded-2xl px-10 pt-4 flex items-center gap-10">
                <div className="flex flex-col items-center gap-3 cursor-pointer" onClick={() => setActiveTab('Department')}>
                    <p>Department</p>
                    <div
                        className={cn(
                            "bg-[#FF7501] rounded-tl-[12px] rounded-tr-[12px] min-w-32 min-h-1 transition-all",
                            activeTab !== 'Department' && "invisible"
                        )}
                    ></div>
                </div>
                <div className="flex flex-col items-center gap-3 cursor-pointer" onClick={() => setActiveTab('Category')}>
                    <p>Category</p>
                    <div
                        className={cn(
                            "bg-[#FF7501] rounded-tl-[12px] rounded-tr-[12px] min-w-24 min-h-1 transition-all",
                            activeTab !== 'Category' && "invisible"
                        )}
                    ></div>
                </div>
                <div className="flex flex-col items-center gap-3 cursor-pointer" onClick={() => setActiveTab('Sector')}>
                    <p>Sector</p>
                    <div
                        className={cn(
                            "bg-[#FF7501] rounded-tl-[12px] rounded-tr-[12px] min-w-24 min-h-1 transition-all",
                            activeTab !== 'Sector' && "invisible"
                        )}
                    ></div>
                </div>
                <div className="flex flex-col items-center gap-3 cursor-pointer" onClick={() => setActiveTab('SDG')}>
                    <p>SDG</p>
                    <div
                        className={cn(
                            "bg-[#FF7501] rounded-tl-[12px] rounded-tr-[12px] min-w-18 min-h-1 transition-all",
                            activeTab !== 'SDG' && "invisible"
                        )}
                    ></div>
                </div>
            </div>
                ...
                <div className={cn("w-full h-full bg-white rounded-2xl flex items-center justify-center", maxHeight)}>
                    <div className="flex flex-col items-center justify-center gap-10">
                        ...
                        <div className="flex flex-col">
                            <p className="text-center font-semibold text-base text-label-dark">{`No ${activeTab} Found`}</p>
                            <p className="text-base text-text-hint font-normal">Please add a new <span className="lowercase">{activeTab}</span> to view related data.</p>
                        </div>
                        <CustomButton
                            value={`Create ${activeTab}`}
                            leftIcon={<Plus className="w-4.5 h-4.5" />}
            ...
            {showCEModal &&
                <CrudModal
                    title={mode === 'create' ? `Create New ${activeTab}` : `Edit ${activeTab}`}
